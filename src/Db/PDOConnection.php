...
use PDO;
use PDOStatement;
use Yng\Db\Exception\BindParamException;
use Yng\Db\Exception\DbEventException;
use Yng\Db\Exception\DbException;
use Yng\Db\Exception\PDOException;
use Yng\Model;
...
abstract class PDOConnection extends Connection
{
    ...
    public function aggregate(BaseQuery $query, string $aggregate, $field, bool $force = false, bool $one = true)
    {
        ...
        $field = $aggregate . '(' . (!empty($distinct) ? 'DISTINCT ' : '') . $this->builder->parseKey($query, $field, true) . ') AS Yng_' . strtolower($aggregate);

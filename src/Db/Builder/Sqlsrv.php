...
class Sqlsrv extends Builder
{
     ...
     * @var string
     */
    protected $selectSql = 'SELECT T1.* FROM (SELECT Yngphp.*, ROW_NUMBER() OVER (%ORDER%) AS ROW_NUMBER FROM (SELECT %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%) AS Yngphp) AS T1 %LIMIT%%COMMENT%';
    /**

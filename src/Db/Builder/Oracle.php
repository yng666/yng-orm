...
{
    protected $selectSql = 'SELECT * FROM (SELECT Yngphp.*, rownum AS numrow FROM (SELECT  %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%%ORDER%) Yngphp ) %LIMIT%%COMMENT%';

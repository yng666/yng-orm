...
class HasMany extends Relation
{
    ...
    public function eagerlyResult(Model $result, string $relation, array $subRelation = [], Closure $closure = null, array $cache = []): void
    {
        ...
        if (isset($result->$localKey)) {
            $pk   = $result->$localKey;
            $data = $this->eagerlyOneToMany([[$this->foreignKey, '=', $pk],], $subRelation, $closure, $cache);
    ...
    public function relationCount(Model $result, Closure $closure = null, string $aggregate = 'count', string $field = '*', string &$name = null)
    {
        ...
        return $this->query
            ->where($this->foreignKey, '=', $result->$localKey)->$aggregate($field);
    }
    ...
    protected function eagerlyOneToMany(array $where, array $subRelation = [], Closure $closure = null, array $cache = []): array
    {
        ...
        $list = $this->query->where($where)->cache($cache[0] ?? false, $cache[1] ?? null, $cache[2] ?? null)->with($subRelation)->select();
